...
public productsAll: any;
  ...
  constructor(public snackBar: MatSnackBar,private prod: ProductsService) {
  ...
  public getItems(): Observable<CartItem[]> {
   ...
   public addToCart(product: Product, quantity: number) {
    // console.log('produitId :', this.productsAll.id);
    let message, status;
     ...
     if(!hasItem) {
      ...
      products.push(item);
      message = 'L\' article ' + product.name + '  à été ajouter au panier.';
      status = 'success';
      this.snackBar.open(message, '×', { panelClass: [status], verticalPosition: 'top', duration: 3000 });
  }
